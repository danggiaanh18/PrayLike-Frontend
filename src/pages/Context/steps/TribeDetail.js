// ./src/pages/Context/steps/TribeDetail.js
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import './TribeDetail.css';
...
// ✅ Tribe Descriptions (COPY từ TribesIntroduction.js - ĐẦY ĐỦ 12 TRIBES)
...
// 支派顏色對照表
const tribeColors = {
  1: '#D4AF37', 2: '#C85A54', 3: '#6B8E23', 4: '#8FBC8F',
  5: '#5F9EA0', 6: '#4682B4', 7: '#708090', 8: '#4169E1',
  9: '#6495ED', 10: '#87CEEB', 11: '#DDA0DD', 12: '#9370DB',
};

const TribeDetail = ({ tribeId, tribeName, tribeIcon, onBack, onTribeSelected, user }) => {
  const navigate = useNavigate();
  const [tribeDetails, setTribeDetails] = useState(null);
  ...
  useEffect(() => {
    const fetchTribeDetails = async () => {
      if (!tribeId) {
        setError('No tribe information found');
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const tribeInfo = apiData.tribes.find(t => t.code === tribeId);
        
        const tribeDesc = tribeDescriptions[tribeId] || {};
        ...
        if (tribeInfo) {
          setTribeDetails({
            ...
            name: tribeName,
            icon: tribeIcon,
            description: tribeInfo.label,
            color: tribeColors[tribeId] || '#808080',
            status: 'Active',
        ...
        console.error("❌ 獲取支派資訊失敗:", e);
        setError(`Unable to load tribe information: ${e.message}`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <AppBackground backgroundColor="#2D3656">
        <div className="tribe-detail-wrapper">
          <div className="tribe-info-container">
            <div className="loading-container">
              <div className="loading-spinner"></div>
              <p>Loading Tribe Information...</p>
            </div>
          </div>
        </div>
      </AppBackground>
            ...
            <div className="tribe-detail-content">
              <div className="tribe-detail-header-row">
                <div 
                  ...
                  style={{ backgroundColor: tribeDetails.color }}
                >
                  {tribeDetails.icon ? (
                    <img 
                      src={tribeDetails.icon} 
                      alt={tribeDetails.name}
                      onError={handleImageError}
                    />
                  ) : (
                    <div className="tribe-icon-placeholder">
                      {tribeDetails.name.charAt(0)}
                    </div>
                  )}
                </div>

                <div className="tribe-detail-text-section">
                  <h2 className="tribe-detail-name">
                    {tribeDetails.name} {tribeDetails.badge}
                  </h2>
                  <div className="tribe-detail-status">Status: {tribeDetails.status}</div>
                </div>
              </div>
              
              {/* ✅ CHỈ HIỂN THỊ FULL DESCRIPTION - BỎ PHẦN THỐNG KÊ */}
              <div className="tribe-detail-info">
                <div className="tribe-full-description">
                  <pre style={{ 
                    whiteSpace: 'pre-wrap', 
                    fontFamily: 'inherit', 
                    fontSize: '14px', 
